    ...
    if (i < 3 && firstline.length <= 2) {
      ...
      if (secondlineleft.length == 2 || secondlineright == 2) {
        ...
        if (array[i] > edge && array[i] < firstline[1]) {
          if (array[i] < secondlineright[0] && thirdlinerightleft.length < 2) {
        ...
        if (array[i] > edge && array[i] > firstline[1]) {
          if (array[i] < secondlineright[1] && thirdlinerightright.length < 2) {
  ...
  for (var i = 1; i < resultarray.length; i++) {
    if (i > 6 && i <= 14 && resultarray[i] != "NIL") {
      ...
      if (i % 2 == 1 && colors[i] == "red") {
        ...
        if (colors[(i - 1) / 2] == "red" && colors[(i - 1) / 2 - 1] == "red") {
...
export default function RedBlackTree() {
  return (
    <form>
      <h1 className="highpad">Piros-Fekete Fa</h1>
      <div class="form-group">
        <div className="normalpad">
          ...
          <Button variant="success" onClick={Rbtadd}>
            Kiszámol
          </Button>
        </div>
      </div>
